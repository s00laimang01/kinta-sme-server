...
export const checkIfUserIsAuthenticated = async (adminAccess = false) => {
  ...
  if (adminAccess && user?.role === "user") {
    throw new Error("UNAUTHORIZED: You are not allow to access this endpoint");
  }
...
export const getDedicatedAccount = async () => {
  const res =
    await api.get<apiResponse<dedicatedAccountNumber>>("/account/me/");
